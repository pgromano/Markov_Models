...
class BaseMicroMSM(object):
        ...
        if N is None:
            if centroids is None:
                ...
                if method.lower() == 'kneighborsclassifier':
                    self.labels = mm.models.classifier._KNeighborsClassifier(self, **kwargs)
                if method.lower() == 'gaussiannb':
                    self.labels = mm.models.classifier._GaussianNB(self, **kwargs)
        else:
    ...
    def predict(self, data, method='KNeighborsClassifier', **kwargs):
        ...
        if method.lower() == 'kneighborsclassifier':
            return mm.models.classifier._KNeighborsClassifier(self, data=data, **kwargs)
        if method.lower() == 'gaussiannb':
            return mm.models.classifier._GaussianNB(self, data=data, **kwargs)
...
class BaseMacroMSM(object):
    ...
    def predict(self, data, method='KNeighborsClassifier', **kwargs):
        ...
        if method.lower() == 'kneighborsclassifier':
            return mm.models.classifier._KNeighborsClassifier(self, data=data, labels=self.metastable_labels-1, **kwargs)
        if method.lower() == 'gaussiannb':
            return mm.models.classifier._GaussianNB(self, data=data, labels=self.metastable_labels-1, **kwargs)
