...
class _AgglomerativeClustering(BaseModel, ClusterMixin, TransformerMixin):
    def __init__(self, *args, **kwargs):
        BaseModel.__init__(self, *args, **kwargs)

    def fit(self, fraction=0.5, shuffle=True, **kwargs):
        train = self._training_set(fraction=fraction, shuffle=shuffle)
        ag = AgglomerativeClustering(**kwargs).fit(train)
        self.centroids = ag.subcluster_centers_
        self.labels = [ag.predict(self.data[i]) for i in range(self.n_sets)]
        self.predict = ag.predict
