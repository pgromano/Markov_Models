...
class _HiddenMarkovModels(BaseModel, ClusterMixin, TransformerMixin):
    ...
    def fit(self, N, fraction=0.5, shuffle=True, **kwargs):
        ...
        with warnings.catch_warnings():
            ...
            hmm = _HMM(n_components=N).fit(train)
            self.labels = [hmm.predict(self.data[i]) for i in range(self.n_sets)]
            self.centroids = hmm.means_
            self.covariances = hmm.covars_
            self.score = hmm.score
            self.score_samples = hmm.score_samples
            self.sample = hmm.sample
